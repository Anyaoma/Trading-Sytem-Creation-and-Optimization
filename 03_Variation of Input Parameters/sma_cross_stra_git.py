...
from Strategy_optimization.sma_cross_stra_git import *
...
def run_strategy(symbol, start, end, time_frame='15Min',risk_percent=5, fast_sma=10, slow_sma=20, lookback=10,add_comm_slipp=True, commission_per_share=0.05, slippage_per_share=0.003):
...
def calculate_statistics(returns, data, currency='£'):
    ...
    return stats
def run_symbol(symbol,risk_percent,add_comm_slipp=True, ):
    comm=0.05
    slipp=0.03
    risk_decimal = float(risk_percent)/100
    metrics_final= []

    for fsv in range(1,20):
        for ssv in range(10,30):
            for lbv in [10, 20]:
                if fsv < ssv:
                    _, result_dict = run_strategy(symbol, start='2022-09-07', end='2025-01-07', time_frame='15Min',risk_percent=risk_decimal, fast_sma=fsv, slow_sma=ssv, lookback=lbv,add_comm_slipp=add_comm_slipp, commission_per_share=comm, slippage_per_share=slipp)
                    ...
                    metrics.update({'fast_sma_value':fsv, 'slow_sma_value':ssv,'lookback_value':lbv,'commission_per_share':comm,'slippage_per_share':slipp,'symbol':symbol})
...
def run_sma_crossover():
    ...
    save_dir = './exploration/sma_crossover'
    os.makedirs(save_dir, exist_ok=True)
    for symbol in symbol_list:
        results_df = run_symbol(symbol, risk_percent=5)
